...
const mockOrderedTasksBySectionAtom = atom((get) => {
  return (projectId: string, sectionId: string | null) => {
    ...
    const tasks =
      projectId === "inbox"
        ? allTasks.filter((task) => !task.projectId || task.projectId === INBOX_PROJECT_ID)
        : allTasks.filter((task) => task.projectId === projectId)

    // Get project for section validation
    const project = projectId === "inbox" ? null : allProjects.find((p) => p.id === projectId)
    ...
    return sectionTasks.sort((a, b) => a.createdAt.getTime() - b.createdAt.getTime())
  }
})
...
describe("Ordered Tasks By Section Atom", () => {
  ...
  describe("Valid Section Filtering", () => {
    it("should return only tasks with matching valid section ID", () => {
      const getOrderedTasksForSection = store.get(mockOrderedTasksBySectionAtom)
  ...
  describe("Orphaned Task Handling (Bug Fix)", () => {
    it("should include orphaned tasks (invalid section IDs) in the default section", () => {
      const getOrderedTasksForSection = store.get(mockOrderedTasksBySectionAtom)
      const defaultSectionTasks = getOrderedTasksForSection(TEST_PROJECT_ID_1, DEFAULT_UUID)
    ...
    it("should include orphaned tasks when requesting default section with null", () => {
      const getOrderedTasksForSection = store.get(mockOrderedTasksBySectionAtom)
      const defaultSectionTasks = getOrderedTasksForSection(TEST_PROJECT_ID_1, null)
  ...
  describe("Edge Cases", () => {
    it("should handle project with no sections", () => {
      ...
      const getOrderedTasksForSection = store.get(mockOrderedTasksBySectionAtom)
      const defaultTasks = getOrderedTasksForSection(projectWithNoSections.id, DEFAULT_UUID)
    ...
    it("should handle empty task list", () => {
      ...
      const getOrderedTasksForSection = store.get(mockOrderedTasksBySectionAtom)
      const results = getOrderedTasksForSection(TEST_PROJECT_ID_1, TEST_SECTION_ID_1)
    ...
    it("should handle non-existent project", () => {
      const getOrderedTasksForSection = store.get(mockOrderedTasksBySectionAtom)
      const results = getOrderedTasksForSection("non-existent-project", TEST_SECTION_ID_1)
  ...
  describe("Regression Prevention", () => {
    it("should prevent the original bug: orphaned tasks disappearing completely", () => {
      ...
      const section1Tasks = getOrderedTasksForSection(TEST_PROJECT_ID_1, TEST_SECTION_ID_1)
      const section2Tasks = getOrderedTasksForSection(TEST_PROJECT_ID_1, TEST_SECTION_ID_2)
      const defaultTasks = getOrderedTasksForSection(TEST_PROJECT_ID_1, DEFAULT_UUID)
