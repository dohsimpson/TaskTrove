...
describe("projectTaskCountsAtom", () => {
  ...
  it("should work with task additions", async () => {
    ...
    const initialTotalCounts = Object.values(initialResult).reduce((sum, count) => sum + count, 0)
    ...
    try {
      ...
      const updatedTotalCounts = Object.values(updatedResult).reduce((sum, count) => sum + count, 0)
  ...
  it("should handle the relationship with activeTasksAtom correctly", async () => {
    ...
    const totalCountsFromAtom = Object.values(taskCounts).reduce((sum, count) => sum + count, 0)
  ...
  it("should properly filter archived tasks", async () => {
    ...
    const archivedTasks = allTasks.filter((task: Task) => task.status === "archived")
    const nonArchivedTasks = allTasks.filter((task: Task) => task.status !== "archived")
    ...
    // Verify that task counts are based on non-archived tasks
    if (nonArchivedTasks.length > 0) {
      ...
      expect(totalCountsFromAtom).toBeLessThanOrEqual(nonArchivedTasks.length)
    }
  })
