...
import { createLabelId, createProjectId } from "@/lib/types"
import { quickAddTaskAtom, updateQuickAddTaskAtom, resetQuickAddTaskAtom } from "./dialogs"
...
describe("Quick Add Dialog Atoms", () => {
  ...
  describe("updateQuickAddTaskAtom", () => {
    ...
    it("should not clear fields that are not in the update request", () => {
      ...
      store.set(updateQuickAddTaskAtom, {
        updateRequest: { labels: [createLabelId(labelId1), createLabelId(labelId2)] },
      })

      const updatedTask = store.get(quickAddTaskAtom)
      expect(updatedTask.labels).toEqual([createLabelId(labelId1), createLabelId(labelId2)])
    ...
    it("should handle updating multiple fields at once", () => {
      ...
      expect(task.labels).toEqual([createLabelId(labelIdA), createLabelId(labelIdB)])
