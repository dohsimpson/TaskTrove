 ...
 */
export const currentPomodoroTaskAtom = atom<{ id: TaskId | null; title: string | null }>((get) => {
  const state = get(pomodoroTimerAtom)
  return {
    id: state.taskId,
...
export const startPomodoroAtom = atom(
  null,
  (get, set, params: { task?: Task; sessionType?: "work" | "short-break" | "long-break" }) => {
...
export const pausePomodoroAtom = atom(null, (get, set) => {
  ...
  const newElapsedTime =
    currentState.elapsedTime + (currentState.startTime ? now - currentState.startTime : 0)
...
export const completePomodoroSessionAtom = atom(null, (get, set) => {
  ...
  if (currentState.sessionType === "work") {
    ...
    nextSessionType = newSessionsCompleted % 4 === 0 ? "long-break" : "short-break"
  } else {
...
export const updatePomodoroSettingsAtom = atom(
  ...
  (
    ...
    set,
    updates: Partial<
      Pick<PomodoroTimerState, "workDuration" | "shortBreakDuration" | "longBreakDuration">
    >,
  ) => {
    const currentState = get(pomodoroTimerAtom)
    set(pomodoroTimerAtom, {
