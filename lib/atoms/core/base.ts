 ...
 */
async function handleApiError(response: Response, module: string): Promise<never> {
...
interface MutationConfig<TResponse, TRequest, TOptimisticData = unknown> {
  ...
  optimisticDataFactory?: (variables: TRequest, oldData?: DataFile) => TOptimisticData
  logModule?: string
...
function createMutation<TResponse, TVariables, TOptimisticData = unknown>(
  ...
  >((get) => ({
    ...
    onMutate: async (variables: TVariables) => {
      ...
      const validCurrentData = isValidCacheData(currentData) ? currentData : EMPTY_CACHE_DATA
...
export const dataQueryAtom = atomWithQuery(() => ({
  ...
  queryFn: async () => {
    ...
    if (typeof window === "undefined" || process.env.NODE_ENV === "test") {
      ...
      log.info({ module: "test" }, "Test environment: Using test data structure")
      return {
        tasks: [],
        projects: [
          {
...
export const updateTasksMutationAtom = createMutation<UpdateTaskResponse, TaskUpdateUnion, Task[]>({
  method: "PATCH",
  ...
  optimisticUpdateFn: (tasks: TaskUpdateUnion, oldData: DataFile) => {
    ...
    const updatedTasks = oldData.tasks.map((task: Task) => {
      ...
      if (newTask.completed !== undefined && newTask.completed !== task.completed) {
...
export const createTaskMutationAtom = createMutation<CreateTaskResponse, CreateTaskRequest, Task>({
  method: "POST",
...
export const deleteTaskMutationAtom = createMutation<DeleteTaskResponse, DeleteTaskRequest, Task[]>(
  {
    method: "DELETE",
    ...
    optimisticUpdateFn: (deleteRequest: DeleteTaskRequest, oldData: DataFile) => {
      ...
      const updatedTasks = oldData.tasks.filter((task: Task) => task.id !== deleteRequest.id)
...
>({
  ...
  optimisticDataFactory: (labelData: CreateLabelRequest, oldData?: DataFile) => {
...
>({
  ...
  optimisticDataFactory: (projectData: CreateProjectRequest, oldData?: DataFile) => {
...
>({
  ...
  optimisticUpdateFn: (labelId: DeleteLabelRequest, oldData: DataFile) => {
    ...
    const updatedLabels = oldData.labels.filter((label: Label) => label.id !== labelId.id)
...
export const tasksAtom = atom(
  ...
  async (get, set, tasks: UpdateTaskRequest[]) => {
    try {
      ...
      log.error({ error, module: "tasks" }, "Failed to update tasks in createTaskMutationAtom")
      throw error
    }
  },
)
...
>({
  ...
  optimisticUpdateFn: (projects: ProjectUpdateUnion, oldData: DataFile) => {
    ...
    const newProjectsMap = new Map(projectUpdates.map((project) => [project.id, project]))
...
export const updateLabelsMutationAtom = createMutation<UpdateLabelResponse, Label[], Label[]>({
  method: "PATCH",
...
export const settingsAtom = atom(
  ...
  async (get, set, partialSettings: PartialUserSettings) => {
    try {
      ...
      log.error({ error, module: "settings" }, "Failed to update settings in settingsAtom")
      throw error
    }
  },
)
