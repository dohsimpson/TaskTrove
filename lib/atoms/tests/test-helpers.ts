...
import { INBOX_PROJECT_ID, createTaskId, createSubtaskId, createSectionId } from "../../types"
import { TEST_LABEL_ID_1, TEST_LABEL_ID_2, TEST_LABEL_ID_3 } from "../../utils/test-constants"
 ...
 */
export function createMockLabel(overrides: Partial<Omit<Label, "id">> = {}): Omit<Label, "id"> {
  const defaults = {
...
export function createMockTaskSet(): Task[] {
  ...
  return [
    ...
    createMockTask({
      ...
      subtasks: [
        { id: createSubtaskId(uuidv4()), title: "Subtask 1", completed: true, order: 0 },
        { id: createSubtaskId(uuidv4()), title: "Subtask 2", completed: false, order: 1 },
      ],
    }),
  ]
}
 ...
 */
export function assertLength<T>(array: T[], expectedLength: number, testName: string): void {
  assert(
...
export function assertEqual<T>(actual: T, expected: T, testName: string): void {
  assert(deepEqual(actual, expected), "Objects should be deeply equal", testName, actual, expected)
}

/**
 * Assert that a value is of a specific type
 */
export function assertType(value: unknown, expectedType: string, testName: string): void {
 ...
 */
export function validateLinkedListIntegrity<T extends { id: string; nextId?: string | null }>(
  items: T[],
  testName: string,
): void {
  // Find the first item (no other item points to it)
  const firstItem = items.find((item) => !items.some((other) => other.nextId === item.id))

  assert(!!firstItem, "Linked list should have a first item", `${testName} - Has First Item`)
  ...
  while (current) {
    ...
    if (current.nextId) {
      ...
      assert(!!nextItem, `Next item ${nextId} should exist`, `${testName} - Next Item Exists`)
      current = nextItem
    } else {
      current = undefined
    }
 ...
 */
export function linkedListToArray<T extends { id: string; nextId?: string | null }>(
  items: T[],
): T[] {
  const sorted: T[] = []
