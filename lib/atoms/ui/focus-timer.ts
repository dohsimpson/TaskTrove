 ...
 */
export const isTaskTimerActiveAtom = atom((get) => (taskId: TaskId): boolean => {
  const state = get(focusTimerStateAtom)
 ...
 */
export const focusTimerStatusAtom = atom<"stopped" | "running" | "paused">((get) => {
  const activeTimer = get(activeFocusTimerAtom)
...
export const startFocusTimerAtom = atom(null, (get, set, taskId: TaskId) => {
  ...
  const existingTimerIndex = currentState.activeTimers.findIndex((timer) => timer.taskId === taskId)
...
export const stopFocusTimerAtom = atom(null, (get, set, taskId: TaskId) => {
  const currentState = get(focusTimerStateAtom)
  const newTimers = currentState.activeTimers.filter((timer) => timer.taskId !== taskId)
