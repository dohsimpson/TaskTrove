...
// Import suite types for advanced sound playing
import type { SoundDefinition, SoundEffect } from "./audio-suites"

/**
 * Apply audio effects to a node
 */
const applyEffects = (
  context: AudioContext,
  ...
  if (!effects || effects.length === 0) return sourceNode
  ...
  effects.forEach((effect) => {
    switch (effect.type) {
      case "filter":
        const filter = context.createBiquadFilter()
        // Type-safe handling of filter configuration
        const validFilterTypes = [
          ...
          "notch",
          "allpass",
        ] satisfies readonly BiquadFilterType[]
        const validFilterTypeStrings: readonly string[] = validFilterTypes

        // Type guard for BiquadFilterType
        function isValidBiquadFilterType(type: unknown): type is BiquadFilterType {
          return typeof type === "string" && validFilterTypeStrings.includes(type)
        }

        const filterType = effect.config.type
        if (isValidBiquadFilterType(filterType)) {
          filter.type = filterType
        } else {
          filter.type = "lowpass"
        }

        const frequency = effect.config.frequency
      ...
      case "delay":
        ...
        // Type-safe handling of delay parameters
        const delayTime = effect.config.time
        delay.delayTime.value = typeof delayTime === "number" ? delayTime : 0.1

        const feedbackValue = effect.config.feedback
        feedbackGain.gain.value = typeof feedbackValue === "number" ? feedbackValue : 0.3

        const wetValue = effect.config.wet
        const wetAmount = typeof wetValue === "number" ? wetValue : 0.3
      ...
      case "reverb":
        // Simple reverb using convolution (would need impulse response in real implementation)
        // For now, simulate with multiple delays
        const reverbGain = context.createGain()
        const reverbWet = effect.config.wet
        reverbGain.gain.value = typeof reverbWet === "number" ? reverbWet : 0.3

        const delays = [0.01, 0.03, 0.05, 0.07, 0.09]
        const gains = [0.8, 0.6, 0.4, 0.3, 0.2]

        delays.forEach((delayTime, i) => {
          const d = context.createDelay(1.0)
          const g = context.createGain()
          const roomSize = effect.config.roomSize
          const roomSizeMultiplier = typeof roomSize === "number" ? roomSize : 0.5
          d.delayTime.value = delayTime * roomSizeMultiplier
          g.gain.value = gains[i]
          currentNode.connect(d)
          d.connect(g)
          g.connect(reverbGain)
        })

        const reverbMerger = context.createGain()
        currentNode.connect(reverbMerger)
      ...
      case "distortion":
        ...
        const distortionAmount = effect.config.amount
        const amount = typeof distortionAmount === "number" ? distortionAmount : 0.5
        const samples = 44100
...
/**
 * Play a sound from a sound definition (from suites)
 */
export const playSoundFromDefinition = async (
  ...
  try {
    ...
    const now = context.currentTime
    const { frequencies, durations, waveTypes, volumes, effects, timing, envelope } = definition

    // Determine timing pattern
    const timingPattern = timing?.pattern || "simultaneous"
    const delays = timing?.delays || []
    const rhythm = timing?.rhythm || []

    // Calculate start times based on pattern
    const startTimes: number[] = []

    switch (timingPattern) {
      case "simultaneous":
        frequencies.forEach(() => startTimes.push(now))
        break

      case "sequential":
        let accumulatedTime = 0
        frequencies.forEach((_, i) => {
          startTimes.push(now + accumulatedTime + (delays[i] || 0))
          accumulatedTime += durations[Math.min(i, durations.length - 1)]
        })
        break

      case "staggered":
        frequencies.forEach((_, i) => {
          startTimes.push(now + (delays[i] || i * 0.05))
        })
        break

      case "rhythmic":
        frequencies.forEach((_, i) => {
          startTimes.push(now + (rhythm[i] || i * 0.1))
        })
        break
    }

    // Create and play each tone
    frequencies.forEach((freq, i) => {
      const oscillator = context.createOscillator()
      const gainNode = context.createGain()

      oscillator.type = waveTypes[Math.min(i, waveTypes.length - 1)]
      oscillator.frequency.setValueAtTime(freq, startTimes[i])

      // Connect through effects if any
      const finalNode = applyEffects(context, oscillator, effects)
      finalNode.connect(gainNode)
      gainNode.connect(context.destination)

      // Apply envelope
      const duration = durations[Math.min(i, durations.length - 1)]
      const volume = (volumes[Math.min(i, volumes.length - 1)] || 0.2) * volumeMultiplier
      const env = envelope || { attack: 0.01, decay: 0.05, sustain: 0.7, release: 0.24 }
      ...
      gainNode.gain.setValueAtTime(0, startTimes[i])
      gainNode.gain.linearRampToValueAtTime(volume, startTimes[i] + attackTime)
      gainNode.gain.linearRampToValueAtTime(sustainLevel, startTimes[i] + attackTime + decayTime)
      gainNode.gain.setValueAtTime(
        sustainLevel,
        startTimes[i] + attackTime + decayTime + sustainDuration,
      )
      gainNode.gain.exponentialRampToValueAtTime(0.01, startTimes[i] + duration)

      oscillator.start(startTimes[i])
      oscillator.stop(startTimes[i] + duration)
    })
  } catch (error) {
    log.warn({ error, module: "audio" }, "Failed to play sound from definition")
  }
}
