 ...
 *   - Escape: cancels
 */
export function createKeyboardHandler<T extends HTMLElement>(options: KeyboardHandlerOptions) {
 ...
 */
export function shouldIgnoreNativeKeyboardEvent(event: globalThis.KeyboardEvent): boolean {
  const target = event.target
  return (
    target instanceof HTMLInputElement ||
    target instanceof HTMLTextAreaElement ||
